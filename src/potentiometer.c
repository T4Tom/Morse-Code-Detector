...
// !!!!! Other than the includes, this file is entirely provided by the skeleton. !!!!!
...
void potentiometer_init() { // provided by skeleton
    adc_init();
    adc_gpio_init(POTENTIOMETER_PIN);
}

unsigned int potentiometer_read_raw() { // provided by skeleton
...
int clamp(int value, int min, int max) { // provided by skeleton
    if (value < min) return min;
    if (value > max) return max;
    return value;
}

unsigned int potentiometer_read(unsigned int limit) { // provided by skeleton
